...
const MPU_MARKERS = {
  ...
  content: ['imx6', 'imx6ull', 'imx28', 'ARCH=arm', 'device-tree', 'yocto', 'buildroot'],
};
...
function getCachedDomainInfo() {
  ...
  try {
    const { PROJECT_DIR } = getCore();
    const cachePath = path.join(PROJECT_DIR, '.mcukit', 'state', 'domain-cache.json');
...
function saveDomainCache(result) {
  try {
    const { PROJECT_DIR } = getCore();
    const stateDir = path.join(PROJECT_DIR, '.mcukit', 'state');
...
function scanDir(dir, maxDepth, currentDepth = 0) {
  ...
  const results = [];
  const excludes = ['.git', 'node_modules', 'build', 'output', 'bin', 'obj', '.mcukit', 'sysroots', 'tmp'];
...
function detectMpuPlatform(projectDir) {
  ...
  // Check DTS filenames for i.MX variant
