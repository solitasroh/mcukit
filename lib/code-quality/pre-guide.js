...
const COMPACT_RULES = {
  cpp:        'C/C++: RAII (unique_ptr), no raw new/delete. ranges/algorithms. enum class, constexpr. MISRA if safety-critical. SEC: no strcpy/sprintf, check int overflow, init all vars. CONC: scoped_lock for shared data, atomic for counters.',
  csharp:     'C#: Primary constructors + DI. [ObservableProperty]/[RelayCommand] for WPF. ErrorOr<T> for errors. switch expression over if chains. #nullable enable mandatory. sealed by default. NO BinaryFormatter. IOptions<T> for config.',
  ...
  python:     'Python: Protocol over ABC. match statement for dispatch. dataclass(frozen=True) for value objects. TaskGroup over gather. SEC: no f-string SQL, no shell=True, no pickle untrusted. logging > print. no mutable default args. is None.',
  c:          'C: MISRA C:2012 Required rules. volatile for HW registers. static for file-scope. sizeof(*ptr) idiom. No dynamic alloc in embedded. SEC: strncpy/snprintf only, validate array bounds, init all vars.',
};
...
/**
 * Compress loader markdown output into a single-line [LEARNED] string.
 * Input format (from lib/instinct/loader.js):
 ...
 * @param {number} [maxLen=200] - character budget for the output
 * @returns {string|null} compact form like '[LEARNED] desc(0.92). desc(0.85).' or null
 */
function compactInstinct(raw, maxLen = 200) {
  if (!raw || typeof raw !== 'string') return null;

  const itemRegex = /^- \[[^\]]+\](?:\s\[[^\]]+\])?\s(.+?)\s\(confidence:\s([\d.]+)\)\s*$/;
  const items = [];
  for (const line of raw.split('\n')) {
    const m = line.match(itemRegex);
    if (m) {
      const desc = m[1].trim();
      const conf = parseFloat(m[2]).toFixed(2);
      items.push(`${desc}(${conf})`);
    }
  }
  ...
  for (const item of items) {
    const candidate = out === prefix ? out + item : `${out}. ${item}`;
    if (candidate.length + 1 > maxLen) break; // +1 reserves space for trailing '.'
    out = candidate;
  }
  if (out === prefix) return null;
  return out + '.';
}
...
function generateStructuralGuide(filePath, content) {
  ...
  // (G) Instinct patterns (learned from previous reviews) — graceful skip if unavailable
  try {
    const { loadConvergedPatterns } = require('../instinct/loader');
    const compact = compactInstinct(loadConvergedPatterns(), 200);
    if (compact) parts.push(compact);
  } catch { /* instinct not available — skip */ }
