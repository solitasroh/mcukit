...
function loadContextLevel(level) {
  ...
  switch (level) {
    case 'plugin': {
      const pluginConfigPath = path.join(core.PLUGIN_ROOT, 'rkit.config.json');
      if (fs.existsSync(pluginConfigPath)) {
        try {
          ...
          core.debugLog('ContextHierarchy', 'Failed to load plugin config', { error: e.message });
        }
      }
      return null;
    }
    ...
    case 'user': {
      ...
      if (fs.existsSync(userConfigPath)) {
        try {
          ...
          core.debugLog('ContextHierarchy', 'Failed to load user config', { error: e.message });
        }
      }
      return null;
    }
    ...
    case 'project': {
      const projectConfigPath = path.join(core.PROJECT_DIR, 'rkit.config.json');
      if (fs.existsSync(projectConfigPath)) {
        try {
          ...
          core.debugLog('ContextHierarchy', 'Failed to load project config', { error: e.message });
        }
      }
      return null;
    }
...
function getContextHierarchy(forceRefresh = false) {
  ...
  core.debugLog('ContextHierarchy', 'Hierarchy loaded', {
...
function setSessionContext(key, value) {
  ...
  core.debugLog('ContextHierarchy', 'Session context set', { key });
}
...
function clearSessionContext() {
  _sessionContext = {};
  _hierarchyCache.data = null;
  core.debugLog('ContextHierarchy', 'Session context cleared');
}
