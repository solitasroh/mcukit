...
function getUserConfigDir() {
  ...
  return path.join(homeDir, '.claude', 'mcukit');
}
...
function loadContextLevel(level) {
  ...
  switch (level) {
    case 'plugin': {
      const pluginConfigPath = path.join(common.PLUGIN_ROOT, 'mcukit.config.json');
    ...
    case 'project': {
      const projectConfigPath = path.join(common.PROJECT_DIR, 'mcukit.config.json');
