...
function backupToPluginData() {
  ...
  try {
    ...
    try { require('./debug').debugLog('Paths', 'meta.json write failed', { error: e.message }); } catch (_e) {}
  }
...
function restoreFromPluginData() {
  ...
  try {
    ...
    try {
      ...
      try { require('./debug').debugLog('Paths', 'realpathSync failed', { error: e.message }); } catch (_e) {}
    }
