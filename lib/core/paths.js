...
function getMcukitRoot() { return path.join(getPlatform().PROJECT_DIR, '.mcukit'); }
...
const LEGACY_PATHS = {
  ...
  memory:     () => path.join(getPlatform().PROJECT_DIR, 'docs', '.mcukit-memory.json'),
  snapshots:  () => path.join(getPlatform().PROJECT_DIR, 'docs', '.pdca-snapshots'),
  agentState: () => path.join(getPlatform().PROJECT_DIR, '.mcukit', 'agent-state.json'),
};

const CONFIG_PATHS = {
  mcukitConfig: () => path.join(getPlatform().PROJECT_DIR, 'mcukit.config.json'),
...
function backupToPluginData() {
  ...
  try {
    ...
    history.push({ timestamp: new Date().toISOString(), mcukitVersion: '2.0.0', backed });
