...
/**
 * mcukit.config.json 로드
 * @returns {Object}
 */
function loadConfig() {
  const { globalCache } = getCache();
  const cached = globalCache.get('mcukit-config');
  if (cached) return cached;

  const { PLUGIN_ROOT, PROJECT_DIR } = getPlatform();
  const configPaths = [
    path.join(PROJECT_DIR, 'mcukit.config.json'),
    path.join(PLUGIN_ROOT, 'mcukit.config.json'),
  ];
  ...
  for (const configPath of configPaths) {
    if (fs.existsSync(configPath)) {
      try {
        ...
        globalCache.set('mcukit-config', config);
        return config;
      } catch (e) {
        // 파싱 실패 시 다음 경로 시도
      }
...
/**
 * 전체 mcukit 설정 조회 (환경변수 오버라이드 포함)
 * @param {boolean} [forceRefresh=false]
 * @returns {Object}
 */
function getMcukitConfig(forceRefresh = false) {
  const { globalCache } = getCache();

  if (!forceRefresh) {
    const cached = globalCache.get('mcukit-full-config', 10000);
    if (cached) return cached;
  }
  ...
  const config = {
    ...
    pdca: {
      ...
      automationLevel: process.env.MCUKIT_PDCA_AUTOMATION || getConfig('pdca.automationLevel', 'semi-auto'),
  ...
  globalCache.set('mcukit-full-config', config);
  return config;
}
