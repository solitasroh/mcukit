...
function loadConfig() {
  ...
  const cached = globalCache.get('rkit-config');
  if (cached) return cached;

  const { PLUGIN_ROOT, PROJECT_DIR } = getPlatform();
  const configPaths = [
    path.join(PROJECT_DIR, 'rkit.config.json'),
    path.join(PLUGIN_ROOT, 'rkit.config.json'),
  ];
  ...
  for (const configPath of configPaths) {
    if (fs.existsSync(configPath)) {
      try {
        ...
        globalCache.set('rkit-config', config);
        return config;
      } catch (e) {
        // 파싱 실패 시 다음 경로 시도
      }
...
function getMcukitConfig(forceRefresh = false) {
  ...
  if (!forceRefresh) {
    const cached = globalCache.get('rkit-full-config', 10000);
    if (cached) return cached;
  }
  ...
  globalCache.set('rkit-full-config', config);
  return config;
}
