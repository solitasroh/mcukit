 ...
 * Adapted for rkit: bkit_ → rkit_ MCP tool prefix, bkit.config.json →
 * rkit.config.json canonical version source.
 *
 ...
 *
 * Canonical source: rkit.config.json:version. Cross-checks:
 *   - .claude-plugin/plugin.json:version
 *   - README.md badge `Version-X.Y.Z`
 *   - CHANGELOG.md latest `## [X.Y.Z]` header
 *   - hooks.json description string (approximate)
 *
 * @returns {Promise<{canonical:string|null, rkitConfig:string|null, pluginJson:string|null, readme:string|null, changelog:string|null, hooksJson:string|null, mismatches:Array<{file:string, field:string, declared:string|null}>}>}
 */
async function scanVersions() {
  ...
  const rkitConfig = readJSONField('rkit.config.json', 'version');
  ...
  return { canonical, rkitConfig, pluginJson, readme, changelog, hooksJson, mismatches };
}
