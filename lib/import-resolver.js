...
function resolveVariables(importPath) {
  ...
  return importPath
    .replace(/\$\{PLUGIN_ROOT\}/g, core.PLUGIN_ROOT)
    .replace(/\$\{PROJECT\}/g, core.PROJECT_DIR)
...
function loadImportedContent(absolutePath) {
  ...
  try {
    if (!fs.existsSync(absolutePath)) {
      core.debugLog('ImportResolver', 'Import file not found', { path: absolutePath });
      return '';
    }
    ...
    core.debugLog('ImportResolver', 'Failed to load import', { path: absolutePath, error: e.message });
    return '';
  }
}
...
function resolveImports(frontmatter, sourceFile) {
  ...
  core.debugLog('ImportResolver', 'Resolving imports', {
    sourceFile,
