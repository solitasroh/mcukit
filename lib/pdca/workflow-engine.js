...
const WORKFLOW_STATE_DIR = '.rkit/state/workflows';
...
function selectWorkflow(feature, level, projectRoot) {
  ...
  if (feature.startsWith('hotfix-') || feature.startsWith('fix-')) {
    ...
    if (hotfix) {
      try { return loadWorkflowFile(hotfix.path); } catch (_e) { /* fall through */ }
    }
  }
  ...
  if (level === 'Enterprise') {
    ...
    if (enterprise) {
      try { return loadWorkflowFile(enterprise.path); } catch (_e) { /* fall through */ }
    }
  }
  ...
  if (defaultWf) {
    try { return loadWorkflowFile(defaultWf.path); } catch (_e) { /* fall through */ }
  }

  // 4. First available
  if (workflows.length > 0) {
    try { return loadWorkflowFile(workflows[0].path); } catch (_e) { return null; }
  }
