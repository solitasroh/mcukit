 ...
 * State stored in .mcukit/state/workflows/{feature}.json.
 */
...
function _getDoLockPath() {
  ...
  return path.join(PROJECT_DIR, '.mcukit', 'runtime', 'do-lock.json');
}
...
function _getFeatureWorkflowPath(feature) {
  const { PROJECT_DIR } = getCore();
  return path.join(PROJECT_DIR, '.mcukit', 'state', 'workflows', `${feature}.json`);
}
