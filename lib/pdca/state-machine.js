...
const EVENTS = [
  ...
  'REPORT_DONE', 'FORCE_REPORT', 'ARCHIVE',
  'REJECT',
...
const TRANSITIONS = [
  ...
  // ── Special transitions ──
  {
    from: 'check', event: 'FORCE_REPORT', to: 'report',
...
const GUARDS = {
  ...
  guardMatchRatePass(ctx) {
    ...
    const threshold = getConfig('pdca.matchRateThreshold', 95);
  ...
  guardResumeAvailable(ctx) {
    ...
    const resumePath = path.join(
      PROJECT_DIR, '.rkit', 'state', 'resume', `${ctx.feature}.resume.json`
    );
  ...
  guardCheckpointExists(ctx) {
    const { PROJECT_DIR } = getCore();
    const cpDir = path.join(PROJECT_DIR, '.rkit', 'checkpoints');
    if (!fs.existsSync(cpDir)) return false;
    try {
      const files = fs.readdirSync(cpDir);
      return files.some(f => f.includes(ctx.feature));
    } catch (e) {
      getCore().debugLog('PDCA-SM', 'Checkpoint directory read failed', { feature: ctx.feature, error: e.message });
      return false;
    }
  },
...
const ACTIONS = {
  ...
  createCheckpoint(ctx, _event) {
    ...
    try {
      ...
      debugLog('PDCA-SM', 'Checkpoint creation failed', { feature: ctx.feature, error: e.message });
    }
  },
  ...
  saveResumePoint(ctx, _event) {
    ...
    const resumeDir = path.join(PROJECT_DIR, '.rkit', 'state', 'resume');
    try {
      ...
      debugLog('PDCA-SM', 'Resume point save failed', { feature: ctx.feature, error: e.message });
    }
  },
  ...
  restoreFromResume(ctx, _event) {
    ...
    try {
      ...
      debugLog('PDCA-SM', 'Resume restore failed', { feature: ctx.feature, error: e.message });
    }
  },
  ...
  restoreCheckpoint(ctx, _event) {
    ...
    try {
      ...
      debugLog('PDCA-SM', 'Checkpoint restore failed', { feature: ctx.feature, error: e.message });
    }
  },
