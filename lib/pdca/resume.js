 ...
 *
 * Schema: .rkit/state/resume/{feature}.resume.json
 */
 ...
 * @returns {string} Absolute path to .rkit/state/resume/
 */
    ...
    for (const file of files) {
      ...
      try {
        ...
        try { fs.unlinkSync(filePath); } catch (unlinkErr) {
          getCore().debugLog('PDCA', 'Failed to delete corrupted resume file', { file: filePath, error: unlinkErr.message });
        }
        cleaned.push(file);
      }
    }
