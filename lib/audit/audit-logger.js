...
/**
 * Recursive sanitization with depth limit (PR #6 review I-1).
 * Walks nested objects up to MAX_DEPTH=3 redacting sensitive keys at every level.
 * Arrays are sanitized element-wise; primitives passthrough with length cap.
 * Prior implementation only checked root + one nested level — keys at depth ≥ 2
 * (e.g. `details.outer.inner.password`) leaked unredacted.
 *
 * @param {*} value - Any value (primitive | object | array | undefined).
 *                    Functions and Symbols return undefined (not serialized).
 * @param {number} depth - Current recursion depth (0 = root, MAX = 3).
 *                         At MAX, '[truncated-depth]' sentinel is returned.
 * @returns {*} Sanitized value, '[truncated-depth]' when depth limit hit,
 *              '[non-serializable]' on cycle/getter throw, or undefined
 *              for Function/Symbol/unknown types.
 */
const SANITIZE_MAX_DEPTH = 3;
function sanitizeValue(value, depth) {
  if (value === null || typeof value === 'number' || typeof value === 'boolean') {
    return value;
  }
  if (typeof value === 'string') {
    return value.length > DETAILS_VALUE_MAX_CHARS
      ? value.slice(0, DETAILS_VALUE_MAX_CHARS) + '…[truncated]'
      : value;
  }
  if (depth >= SANITIZE_MAX_DEPTH) return '[truncated-depth]';
  if (Array.isArray(value)) {
    try { return value.map((v) => sanitizeValue(v, depth + 1)); }
