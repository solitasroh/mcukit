 ...
 * @version 2.0.0
 *
 * Records every significant decision made during rkit operations,
 * including alternatives considered and rationale for chosen option.
 * Storage: .rkit/decisions/YYYY-MM-DD.jsonl (one JSON object per line)
 *
 ...
 * @returns {string} Absolute path to .rkit/decisions/
 */
function getDecisionsDir() {
  return path.join(getPlatform().PROJECT_DIR, '.rkit', 'decisions');
}
