...
const METRIC_SPECS = {
  ...
  M11: { id: 'M11', name: 'Architecture Compliance', collector: 'design-validator', unit: '0-100', direction: 'higher' },
};
...
function analyzeTrend(feature) {
  ...
  const metricIds = ['M1', 'M2', 'M3', 'M7', 'M9', 'M10', 'M11'];
  ...
  // Alarm 7: Architecture Compliance moving avg < 90
  if (result.movingAverages.M11 != null && result.movingAverages.M11 < 90) {
    result.alarms.push({
      type: 'architecture_compliance_low',
      message: `Architecture compliance moving average is ${result.movingAverages.M11} (< 90)`,
      recommendation: 'Architecture review failed. Refactor design to align with global architecture map without duplicating code.',
    });
  }
