 ...
 * destructive operation handling, and rkit.config.json integration.
 */
...
function _getTrustScore() {
  try { return require('./trust-engine').getScore(); } catch (_e) { return 40; }
}
...
function _getControlStatePath() {
  ...
  return path.join(PROJECT_DIR, '.rkit', 'runtime', 'control-state.json');
}
...
function setLevel(level, options = {}) {
  ...
  if (!force) {
    ...
    } catch (_e) { /* guard-mode not available */ }
  }
