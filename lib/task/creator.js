...
function generateTaskGuidance(phase, feature, blockedByPhase = '') {
  ...
  if (['plan', 'design', 'do'].includes(phase)) {
    try {
      ...
      if (phase === 'design') {
        guidance += '\n\n**CRITICAL DESIGN CONSTRAINT**: You MUST provide a Mermaid diagram representing the holistic architecture integrating with the listed existing modules (or establishing the base if zero-codebase). Avoid writing implementations here; focus on interfaces and structure.';
      } else if (phase === 'do') {
