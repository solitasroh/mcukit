 ...
 * See ENH-85 for migration details.
 *
 * @version 1.6.0
...
function forkContext(skillOrAgentName, options = {}) {
  ...
  const currentStatus = pdcaStatus.getPdcaStatusFull(true);
  ...
  core.debugLog('ContextFork', 'Context forked', {
    forkId,
    skillOrAgentName,
...
function updateForkedContext(forkId, updates) {
  const fork = _forks.get(forkId);
  if (!fork) {
    core.debugLog('ContextFork', 'Fork not found', { forkId });
    return;
  }

  Object.assign(fork.forkedState, updates);
  core.debugLog('ContextFork', 'Fork updated', { forkId, keys: Object.keys(updates) });
}
...
function mergeForkedContext(forkId, options = {}) {
  ...
  if (!fork.mergeResult) {
    ...
    core.debugLog('ContextFork', 'Merge skipped (mergeResult: false)', { forkId });
  ...
  core.debugLog('ContextFork', 'Context merged', {
    forkId,
...
function discardFork(forkId) {
  _forks.delete(forkId);
  core.debugLog('ContextFork', 'Fork discarded', { forkId });
}
...
function clearAllForks() {
  ...
  _forks.clear();
  core.debugLog('ContextFork', 'All forks cleared', { count });
}
