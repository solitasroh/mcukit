...
Also collect from `.mcukit/state/`:
...
Categories:
- **Skill**: Add or improve an mcukit skill
- **Config**: Change `.mcukit/` configuration
...
Save structured learnings to `.mcukit/state/learnings.json`:
...
---
Generated by mcukit retro skill
```
...
When starting a new feature with `/pdca plan`, check `.mcukit/state/learnings.json` for:
