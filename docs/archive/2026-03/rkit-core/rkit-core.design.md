...
> **Date**: 2026-03-22
> **Status**: Draft
> **Plan Reference**: `docs/01-plan/features/rkit-core.plan.md` v0.3
...
│  session-start.js                                                │
│       │                                                          │
│       ├─ migration.js ──→ paths.js (.rkit/ 경로)               │
│       ├─ restore.js   ──→ state-store.js (백업 복원)              │
│       ├─ context-init.js → config.js (rkit.config.json 로드)   │
...
```
1. rkit.config.json의 domain.override 확인 → 있으면 즉시 반환
 ...
 *
 * 탐색 순서:
 * 1. rkit.config.json의 mcu.toolchain 값
 ...
 * @param {Object} previous - 이전 빌드 분석 결과 (.rkit/state/build-history.json)
 ...
 * @param {Object} budget - 메모리 예산 (rkit.config.json 또는 memory-budget.yaml)
...
// 프로젝트 레벨 (bkit: Starter/Dynamic/Enterprise → rkit: L1/L2/L3)
const LEVELS = {
  L1_BASIC: 'L1_Basic',
 ...
 * @description 프로젝트 레벨 감지 (bkit의 Starter/Dynamic/Enterprise → rkit의 L1/L2/L3)
 * @changed 전면 재작성 (bkit 웹 마커 → rkit 임베디드/데스크톱 마커)
 */
...
function handleDomainDestructiveCheck(input) {
  ...
  for (const { pattern, reason } of patterns) {
    if (command.toLowerCase().includes(pattern.toLowerCase())) {
      outputBlock(`[rkit] ${reason}. 이 명령은 수동 확인이 필요합니다.`);
      return true;
    }
  }
  return false;
...
Step 7: 기본 Skills 이식
  ...
  └── skills/rkit-rules/SKILL.md (bkit-rules 기반 재작성)
...
```bash
#!/bin/bash
# rename-bkit-to-rkit.sh
# bkit 코어 모듈을 rkit으로 이름 치환
...
find "$DST_DIR/lib" -name "*.js" -exec sed -i \
  ...
  -e 's/\.bkit\//\.rkit\//g' \
  -e "s/'bkit'/'rkit'/g" \
  -e 's/bkit\.config/rkit\.config/g' \
  -e 's/bkit-memory/rkit-memory/g' \
...
| 2 | `rkit.config.json` | bkit 기반 커스터마이즈 | High |
| 3 | `lib/core/platform.js` | 이름 치환 | Low |
| 4 | `lib/core/config.js` | 이름 치환 | Low |
| 5 | `lib/core/paths.js` | 경로 치환 (.bkit→.rkit) | Low |
...
| 24 | `skills/rkit-rules/SKILL.md` | bkit-rules 기반 재작성 | High |
...
| # | 시나리오 | 검증 항목 | 기대 결과 |
|---|---------|-----------|-----------|
| T1 | STM32 프로젝트에서 rkit 세션 시작 | 도메인 감지 | `domain: mcu, platform: stm32` |
