...
- 마스터 기획서: `docs/01-plan/rkit-master-plan.md`
...
│   │   ├── index.js
│   │   ├── platform.js           # 플랫폼 감지 (rkit 적응)
│   │   ├── config.js             # rkit.config.json 로더
│   │   ├── paths.js              # .rkit/ 상태 파일 레지스트리
...
│
├── servers/                      # MCP 서버
│   ├── rkit-pdca-server/       # PDCA 관리 (bkit 이식)
│   └── rkit-analysis-server/   # 도메인별 분석 도구
│
├── evals/                        # 스킬 평가 (bkit 이식)
│   ├── config.json
│   ├── runner.js
│   └── reporter.js
│
├── rkit.config.json            # 통합 설정
├── rkit.config.schema.json     # 설정 스키마
...
```javascript
// rkit.config.json - quality.domainThresholds
{
  "mcu": {
...
| FR-04 | rkit.config.json 설정 체계 | 공통 | High |
...
**목표**: bkit 코어를 rkit으로 이식하고, 3개 도메인 자동 감지 동작
...
| rkit.config.json | bkit 기반 | 커스터마이즈 | 2 |
...
| MCP 서버 (rkit-pdca, rkit-analysis) | 신규 |
| 스킬 평가 (evals/) | bkit 이식 |
...
| 이름 컨벤션 | BKIT→MCUKIT 전체 변경 | **MCUKIT으로 통일** | .rkit/, MCUKIT_*, rkit.config.json |
| 상태 디렉토리 | .rkit/ 단일 | **.rkit/{state,runtime,audit,...}** | bkit v1.5.8 구조 그대로 이식 |
...
> 이 섹션은 bkit-claude-code v2.0.0 심층 리서치 결과를 기반으로 rkit에서의 이식/변경 범위를 정밀하게 정의합니다.
...
// rkit 변환
{
  "name": "rkit",
  "version": "0.1.0",
  "displayName": "rkit — AI Native Embedded Development Kit",
...
### 10.2 Skill SKILL.md Frontmatter 포맷 (rkit 적응)
...
### 10.3 Agent Frontmatter 포맷 (rkit 적응)
...
| # | Hook Event | bkit Script | rkit 변경 사항 |
|---|-----------|-------------|-----------------|
...
```
rkit SessionStart Flow:
  1. migration.js       - 레거시 경로 마이그레이션
  2. restore.js         - PLUGIN_DATA 백업 복원
  3. context-init.js    - Context Hierarchy 초기화, .rkit/ 디렉토리 생성
...
#### rkit-pdca-server (bkit-pdca-server 이식 + 확장)

| 도구 | bkit 원본 | rkit 변경 |
|------|-----------|-------------|
| rkit_pdca_status | bkit_pdca_status | 이름만 변경 |
| rkit_pdca_history | bkit_pdca_history | 이름만 변경 |
| rkit_feature_list | bkit_feature_list | 이름만 변경 |
| rkit_feature_detail | bkit_feature_detail | 도메인 필드 추가 |
| rkit_plan_read | bkit_plan_read | 이름만 변경 |
| rkit_design_read | bkit_design_read | 이름만 변경 |
| rkit_analysis_read | bkit_analysis_read | 이름만 변경 |
| rkit_report_read | bkit_report_read | 이름만 변경 |
| rkit_metrics_get | bkit_metrics_get | 도메인별 메트릭 추가 |
| rkit_metrics_history | bkit_metrics_history | 이름만 변경 |

#### rkit-analysis-server (bkit-analysis-server 이식 + 확장)

| 도구 | bkit 원본 | rkit 변경 |
|------|-----------|-------------|
| rkit_code_quality | bkit_code_quality | MISRA/DTS/C# 분석 추가 |
| rkit_gap_analysis | bkit_gap_analysis | 도메인별 갭 항목 |
| rkit_regression_rules | bkit_regression_rules | 이름만 변경 |
| rkit_checkpoint_list | bkit_checkpoint_list | 이름만 변경 |
| rkit_checkpoint_detail | bkit_checkpoint_detail | 이름만 변경 |
| rkit_audit_search | bkit_audit_search | 이름만 변경 |
| **rkit_memory_report** | ★ 신규 | Flash/RAM 사용량 시계열 |
| **rkit_domain_info** | ★ 신규 | 감지된 도메인/플랫폼 정보 |

### 10.7 템플릿 매핑 (bkit → rkit)
...
| Phase | bkit (Web) | rkit (Embedded/Desktop) |
|-------|-----------|--------------------------|
...
### 10.8 이름 변경 체크리스트 (bkit → rkit)

| 범위 | bkit | rkit |
|------|------|--------|
| 환경변수 | `BKIT_*` | `MCUKIT_*` |
| 상태 디렉토리 | `.bkit/` | `.rkit/` |
| 설정 파일 | `bkit.config.json` | `rkit.config.json` |
| 메모리 파일 | `.bkit/state/memory.json` | `.rkit/state/memory.json` |
| 상태 파일 | `.bkit/state/pdca-status.json` | `.rkit/state/pdca-status.json` |
| 캐시 키 | `bkit-config` | `rkit-config` |
| 에러 코드 | `BKIT_PDCA_*`, `BKIT_STATE_*` | `MCUKIT_PDCA_*`, `MCUKIT_STATE_*` |
| 플러그인 이름 | `bkit` | `rkit` |
| MCP 도구 접두사 | `bkit_*` | `rkit_*` |
| Output Style 접두사 | `bkit-*` | `rkit-*` |
| 디버그 로그 | `.bkit/debug/` | `.rkit/debug/` |
...
| Paths | paths.js | 10+ | `.bkit/` → `.rkit/`, STATE_PATHS 전체 |
...
bkit의 Eval 프레임워크를 이식하여 rkit 스킬 품질을 측정합니다:
...
```
output-styles/
├── rkit-learning.md             # 입문자용 (설명 상세)
├── rkit-pdca-guide.md           # PDCA 워크플로우 가이드 (표준)
├── rkit-embedded.md             # 임베디드 전문가용 (간결, 레지스터 수준)
└── rkit-pdca-embedded.md        # PDCA + 임베디드 전문가 (고급)
```
...
```
Priority 1 (최저): Plugin Level  - rkit.config.json (플러그인 기본값)
Priority 2:       User Level    - ~/.claude/rkit/user-config.json
Priority 3:       Project Level - PROJECT_DIR/rkit.config.json
...
## 11. rkit.config.json 설계

```json
{
  "$schema": "./rkit.config.schema.json",
  "version": "0.1.0",
