...
| Perspective | Content |
|-------------|---------|
| **Problem** | mcukit은 bkit v2.0.3 기반으로 포킹되었으나, bkit이 v2.0.6까지 3개 패치를 릴리즈하여 Context Anchor, Living Context System, Self-Healing 등 핵심 기능이 미반영. 또한 bkit→mcukit 마이그레이션이 불완전하여 `.bkit/` 런타임 경로, MCP 서버 내부, 함수명, 스킬/에이전트 참조에 bkit 흔적이 다수 잔류. gstack의 `/benchmark`, `/investigate`, `/retro` 등 임베디드에 유용한 기능도 미통합. |
| **Solution** | P0: bkit→mcukit 마이그레이션 완료 (런타임 경로 통일, MCP 서버 치환, 함수명 정리). P1: bkit v2.0.5~v2.0.6 핵심 모듈 이식 (session-guide, lib/context/, self-healing). P2: gstack 미반영 기능 임베디드 특화 통합 (/benchmark, /investigate, /retro). P3: 스킬/에이전트/템플릿 bkit 잔류 참조 전수 치환. |
| **Function/UX Effect** | 런타임 경로 일관성으로 상태 관리 안정성 확보, PRD→코드 컨텍스트 침투율 75-85% 달성, 세션 간 컨텍스트 손실 감소, MCU 빌드 벤치마크/HardFault 조사 프로토콜 제공 |
| **Core Value** | mcukit을 bkit 최신 코어와 동기화하여 기술 부채를 해소하고, 임베디드 도메인 특화 도구를 확장하여 MCU/MPU/WPF 개발자의 생산성을 극대화 |
...
mcukit이 bkit v2.0.3에서 포킹된 이후 발생한 3가지 갭을 해소한다:
...
- mcukit은 bkit-claude-code (Apache 2.0, popup-studio-ai)에서 PDCA 코어를 포팅한 프로젝트
- v0.6.0에서 gstack (Garry Tan)의 safety/quality/security/delivery 패턴을 임베디드 도메인에 적응 통합
- 현재 `.bkit/`과 `.mcukit/` 이중 상태 디렉토리가 공존하며, 실제 런타임은 `.bkit/`을 사용하는 불일치 상태
...
- mcukit 마스터 플랜: `docs/01-plan/mcukit-master-plan.md`
- mcukit 코어 설계: `docs/02-design/features/mcukit-core.design.md`
...
- bkit v2.0.4 Windows Hook 경로 호환성 (mcukit은 Linux 전용)
...
- bkit→mcukit URI 스킴 변경 (`bkit://` → `mcukit://`) 시 외부 소비자 호환성 보장 (현재 외부 소비자 없음)
...
|----|-------------|----------|----------|
| FR-01 | `.bkit/` 런타임 디렉토리를 `.mcukit/`으로 통일, 기존 `.bkit/` 데이터 마이그레이션 | P0-High | Migration |
| FR-02 | MCP 서버 내부 `BKIT_ROOT`, `.bkit/`, `bkit://` URI, 서버명을 `mcukit` 계열로 치환 | P0-High | Migration |
| FR-03 | `lib/pdca/status.js`의 `readBkitMemory()`, `writeBkitMemory()` 함수명 치환 | P0-High | Migration |
| FR-04 | `pdca-status.json`의 `team.stateFile`, `team.eventsFile` 경로를 `.mcukit/` 기준으로 갱신 | P0-High | Migration |
...
| FR-12 | 스킬 13개 내부 `.bkit/` 경로 → `.mcukit/` 치환 | P3-Medium | Cleanup |
| FR-13 | 에이전트 6개 내부 bkit 참조 → mcukit 치환 | P3-Medium | Cleanup |
| FR-14 | 템플릿 7개 bkit 푸터/생성자 표기 → mcukit 치환 | P3-Low | Cleanup |
...
- [ ] `.mcukit/` 단일 런타임 디렉토리로 통일, `.bkit/` 미사용
- [ ] MCP 서버 `mcukit://` URI 스킴으로 전환, 16개 도구 정상 동작
...
|------|--------|------------|------------|
| `.bkit/` → `.mcukit/` 전환 시 기존 상태 데이터 손실 | High | Medium | 마이그레이션 스크립트에 백업+검증 단계 추가, rollback 경로 확보 |
| MCP URI 변경 (`bkit://` → `mcukit://`) 시 Claude Code 캐시 불일치 | Medium | Low | 세션 재시작으로 해결, 하위 호환 alias 검토 |
...
> mcukit은 L3_Advanced 레벨 프로젝트로, lib/core + lib/pdca + lib/domain 계층 분리 아키텍처 사용
...
| MCP URI 스킴 | A) `mcukit://` / B) `bkit://` 유지 | A) `mcukit://` | 브랜딩 일관성, 외부 소비자 없음 |
...
├── core/
│   └── paths.js               # [MOD] .bkit/ → .mcukit/ 경로 통일
...
...
servers/
├── mcukit-pdca-server/
│   ├── package.json           # [MOD] name: "mcukit-pdca-server"
│   └── index.js               # [MOD] MCUKIT_ROOT, mcukit://, 전수 치환
├── mcukit-analysis-server/
│   ├── package.json           # [MOD] name: "mcukit-analysis-server"
│   └── index.js               # [MOD] 전수 치환
```
...
| Step | Task | Files | Est. |
|------|------|-------|------|
| 1-1 | `.bkit/` 상태 데이터를 `.mcukit/`으로 복사+검증 | `.bkit/` → `.mcukit/` | 소 |
...
| 1-6 | `pdca-status.json` team 경로 갱신 | `.mcukit/state/pdca-status.json` | 소 |
| 1-7 | SessionStart hook에서 `.bkit/` → `.mcukit/` 마이그레이션 로직 | `hooks/startup/migration.js` | 소 |

### Phase 2: P1 — bkit v2.0.5~v2.0.6 이식 (Day 2-3)

| Step | Task | Files | Est. |
|------|------|-------|------|
| 2-1 | `lib/pdca/session-guide.js` 이식 + mcukit 적응 | 신규 1개 | 중 |
...
| 소비자 | 영향 | 대응 |
|--------|------|------|
| SessionStart Hook | `.bkit/` 경로 읽기 → `.mcukit/` 전환 필요 | migration.js에서 자동 마이그레이션 |
| unified-stop.js | `.bkit/` 상태 파일 참조 | paths.js 중앙 경로 사용으로 자동 해결 |
| 기존 PDCA 3개 피처 (pdca, startup, domain) | `.bkit/state/pdca-status.json` 참조 | 데이터 복사 후 `.mcukit/` 통일 |
