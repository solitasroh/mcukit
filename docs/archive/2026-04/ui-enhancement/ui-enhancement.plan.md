...
rkit lib/ui 모듈을 세 방향으로 개선한다:
1. **Dual Output**: ANSI 터미널 표시(stderr)와 LLM 컨텍스트(additionalContext)를 분리
2. **Config 연동**: 하드코딩된 임계값/폭/단축키를 rkit.config.json에서 로드
...
| FR-04 | P1 | Config 파라미터화 | 임계값, 바 폭, 컬럼폭, 단축키를 rkit.config.json ui 섹션에서 로드 |
