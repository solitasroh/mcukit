...
/**
 * rkit-analysis-server: Code analysis, gap detection, checkpoints, and audit MCP server.
 *
 * Lightweight JSON-RPC 2.0 over stdio — no external dependencies.
 * Reads .rkit/ state, checkpoints, and audit files.
 */
...
const TOOL_HANDLERS = {
  rkit_code_quality: handleCodeQuality,
  rkit_gap_analysis: handleGapAnalysis,
  rkit_regression_rules: handleRegressionRules,
  rkit_checkpoint_list: handleCheckpointList,
  rkit_checkpoint_detail: handleCheckpointDetail,
  rkit_audit_search: handleAuditSearch,
};
  ...
  switch (method) {
    case 'initialize':
      return jsonRpcOk(id, {
        ...
        serverInfo: { name: 'rkit-analysis-server', version: '2.1.0' },
        capabilities: { tools: {} },
      });
...
process.stderr.write('[rkit-analysis-server] Started (pid=' + process.pid + ')\n');
