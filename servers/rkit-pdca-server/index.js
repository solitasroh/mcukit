...
/**
 * rkit-pdca-server: PDCA status, documents, and metrics MCP server.
 *
 * Lightweight JSON-RPC 2.0 over stdio — no external dependencies.
 * Reads .rkit/ state files and docs/ markdown files.
 */
...
const TOOL_HANDLERS = {
  rkit_pdca_status: handlePdcaStatus,
  rkit_pdca_history: handlePdcaHistory,
  rkit_feature_list: handleFeatureList,
  rkit_feature_detail: handleFeatureDetail,
  rkit_plan_read: (args) => handleDocRead('plan', args),
  rkit_design_read: (args) => handleDocRead('design', args),
  rkit_analysis_read: (args) => handleDocRead('analysis', args),
  rkit_report_read: (args) => handleDocRead('report', args),
  rkit_metrics_get: handleMetricsGet,
  rkit_metrics_history: handleMetricsHistory,
};

const RESOURCE_HANDLERS = {
  'rkit://pdca/status': handleResourcePdcaStatus,
  'rkit://quality/metrics': handleResourceQualityMetrics,
  'rkit://audit/latest': handleResourceAuditLatest,
};
  ...
  switch (method) {
    case 'initialize':
      return jsonRpcOk(id, {
        ...
        serverInfo: { name: 'rkit-pdca-server', version: '2.1.0' },
...
process.stderr.write('[rkit-pdca-server] Started (pid=' + process.pid + ')\n');
